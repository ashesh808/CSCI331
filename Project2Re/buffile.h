...
//	the values of the record address depend on the type of file and buffer
{
   public:
	...
	int Open (const char * filename, std::ios_base::openmode MODE); // open an existing file
	int Create (const char * filename, std::ios_base::openmode MODE); // create a new file
	int Close ();
