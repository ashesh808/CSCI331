...
int BufferFile::Open (const char * filename, std::ios_base::openmode mode)
...
// use ios::nocreate to ensure that a file exists
{
	...
	File . open (filename, mode|ios::in|ios::binary|ios::app);
...
int BufferFile::Create (const char * filename, std::ios_base::openmode mode)
...
// use ios::nocreate to ensure that no file exists
{
	...
	File . open (filename, mode|ios::in|ios::out|ios::trunc|ios::binary);
	//File . open (filename, mode|ios::in|ios::out);
	if (!File . good()) 
	{
		File . close();
		return FALSE;
