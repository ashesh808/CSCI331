...
//Read data from a file with a length indicator at the beginning of the file
//to the buffer
int DelimTextBuffer :: Read (istream & stream) 
{
	Clear ();
	stream . read ((char*)&BufferSize, sizeof(BufferSize)); //read the length first
	if (stream.fail()) return FALSE;
	if (BufferSize > MaxBytes) return FALSE; // buffer overflow
	stream . read (Buffer, BufferSize); //read the rest of the record
	return stream . good ();
}

//Write the Buffer contents to a file with a length indicator at the beginning 
//of the file
int DelimTextBuffer :: Write (ostream & stream) const
{
	stream . write ((char*)&BufferSize, sizeof(BufferSize)); 
	//size is stored in binary form
...
int DelimTextBuffer :: Init (char delim, int maxBytes)
 // construct with delimited field
{
	Delim = delim;
	DelimStr[0] = Delim;
	DelimStr[1] = 0;
